...
- ✅ `@[ab-test: タイトル](file_a, file_b)`：汎用A/Bテストコンポーネント（`<situ-ab-test>`）。実装完了。
...
- 拡張(完了): 汎用対話コンポーネント群（投票UI、汎用A/Bテスト）の設計とメディアファイル参照形式（埋め込み/外部リンク）の選択機能の実装。
- 継続(Next): ユーザー用メモ欄（notebook-input）の実装と、設定ファイル管理（config.toml）の導入。
