...
- Frontend Runtime: Vanilla JS / Web Components (No Heavy Frameworks)。すべてのコンポーネント用JS/CSSは、コンバータ実行時に1つのHTML内に完全にインライン展開（パッキング）される。
...
- `@[poll: タイトル](選択肢1, 選択肢2, ...)`：リアルタイム投票コンポーネント（`<situ-poll>`）。
  - 状態モックとペイロード: 通信インフラとUIを分離。投票イベントは `タイムスタンプ` と `コンポーネントID` を含む Phase 3 互換の JSON フォーマットで発火させ、現状は Local Storage に保存・モックする。
