...
* **FastAPIバックエンド:** `src/server.py` はWebSocket (`/ws/sync`) 経由でリアルタイムの視覚同期を処理し、HTTP POST (`/api/data`) で送信されたユーザー入力データをJSONL形式で保存する。サーバーの起動には `uv run python -m src.server` を使用する。
...
## コンバーターと出力の仕様
* **Markdownコンバーターの実行:** `uv run python -m src.main <input.md> -o <output.html>`
