...
- Component Templates Layer: `components/` (Web ComponentsのHTML/JS/CSSをコンポーネント単位で分割管理。ビルド時に動的スキャンし1ファイルへパッキングする「Component-Based Split」アーキテクチャ)。
...
- 実装場所（推奨）:
  - components/mono-group-assignment/
