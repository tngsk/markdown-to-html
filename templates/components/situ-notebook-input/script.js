...
class SituNotebookInput extends HTMLElement {
    constructor() {
        ...
        this.refs = {};
        this.inputId = this.getAttribute("id") || `notebook-${Math.random().toString(36).substr(2, 9)}`;
    ...
    mountTemplate() {
        const template = document.getElementById("situ-notebook-input-template");
        if (!template) {
            console.error("Template #situ-notebook-input-template not found.");
            return;
        }
        ...
        try {
            ...
            console.warn("Local Storage is not available. Notes will not be persisted.", e);
...
if (!customElements.get("situ-notebook-input")) {
    customElements.define("situ-notebook-input", SituNotebookInput);
}
