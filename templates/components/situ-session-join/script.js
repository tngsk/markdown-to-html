class SituSessionJoin extends HTMLElement {
  constructor() {
    super();
    this.attachShadow({ mode: "open" });
    const template = document.getElementById("situ-session-join-template");
    if (template) {
  ...
  checkStatus() {
    try {
      const saved = localStorage.getItem("situ_session_id");
      if (saved) {
  ...
  handleJoin() {
    ...
    try {
      localStorage.setItem("situ_session_id", JSON.stringify(payload));
    } catch (e) {
    ...
    window.dispatchEvent(new CustomEvent("situ-session-changed"));
...
if (!customElements.get("situ-session-join")) {
  customElements.define("situ-session-join", SituSessionJoin);
}
