class SituReaction extends HTMLElement {
  constructor() {
    super();
    this.attachShadow({ mode: "open" });
    const template = document.getElementById("situ-reaction-template");
    if (template) {
  ...
  connectedCallback() {
    ...
    try {
      const stored = localStorage.getItem(`situ_reaction_${this.componentId}`);
      if (stored) {
  ...
  render() {
    ...
    this.options.forEach((option, index) => {
      ...
      btn.className = "situ-reaction-btn";
      ...
      countLabel.className = "situ-reaction-count";
  ...
  handleReaction(option, index) {
    ...
    try {
      localStorage.setItem(`situ_reaction_${this.componentId}`, JSON.stringify(payload));
    } catch (e) {
...
if (!customElements.get("situ-reaction")) {
  customElements.define("situ-reaction", SituReaction);
}
