class SituClock extends HTMLElement {
    constructor() {
        super();
    ...
    connectedCallback() {
        const template = document.getElementById('situ-clock-template');
        if (template) {
            this.shadowRoot.appendChild(template.content.cloneNode(true));
        } else {
            console.error('situ-clock-template not found');
            return;
        }
...
if (!customElements.get('situ-clock')) {
    customElements.define('situ-clock', SituClock);
}
